...
def createPage():
    ...
    @st.cache_data(show_spinner='Cargando Datos... Espere...', persist=True)
    def load_df_lr():
        rutaER = './data/Envios a LR Nestle.xlsx'
    ...
    try:
        ...
        with d1:
            ...
            df_cm = df.copy()   
            st.markdown("<h5 style='text-align: left;'>Predicción de Servicios Activos por Hora en Centro de Monitoreo Nestlé</h5>", unsafe_allow_html=True)
            #n_dias = st.slider('Predicciones (Días):', 1, 30)
            period = n_dias*24
        ...
        with d2:
            ...
            df_lr = df1.copy()
            df2 = envios_lr(df_lr)
            st.markdown("<h5 style='text-align: left;'>Predicción de Envios a LR de Nestlé por Hora</h5>", unsafe_allow_html=True)
            #n_dias = st.slider('Predicciones (Días):', 1, 30)
            period1 = n_dias*24
